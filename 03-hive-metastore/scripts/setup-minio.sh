...
if [[ "$DRY_RUN" == true ]]; then
  echo "  mc cp ${TUTORIAL_DIR}/code/data/sales.csv local/spark-data/input/"
else
  mc cp "${TUTORIAL_DIR}/code/data/sales.csv" local/spark-data/input/
...
echo "  Input:      s3a://spark-data/input/sales.csv"
