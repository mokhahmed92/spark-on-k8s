#!/usr/bin/env bash
# setup-spark-rbac.sh — Create the Spark namespace, ServiceAccount, Role, RoleBinding, and MinIO credentials
...
echo "Step 3: Creating MinIO credentials secret in spark namespace..."
if [[ "$DRY_RUN" == true ]]; then
  echo "  kubectl apply -f ${TUTORIAL_DIR}/manifests/spark/minio-credentials-secret.yaml"
else
  kubectl apply -f "${TUTORIAL_DIR}/manifests/spark/minio-credentials-secret.yaml"
fi

echo "Step 4: Verifying resources..."
if [[ "$DRY_RUN" == true ]]; then
  echo "  kubectl -n spark get sa,role,rolebinding,secret"
else
  kubectl -n spark get sa,role,rolebinding,secret
fi
...
echo "  Secret:          minio-credentials (S3A access)"
